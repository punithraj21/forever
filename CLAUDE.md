...
- `app/page.tsx` — composes the home page (header → countdown cards → Milestones/By the Numbers → Event Details → On the Map → Weather → Advice → quote → footer). The "Punith & Pallavi" heading is rendered by `<PetalEasterEgg />` (tap it to rain petals). The "Our Story" CTA card (gated by `SHOW_OUR_STORY_CTA`) sits before the quote when enabled.
