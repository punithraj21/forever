...
| Path         | Purpose                                                                                  |
| ------------ | ---------------------------------------------------------------------------------------- |
| `/`          | Main page (header, cards, Milestones, By the Numbers, Event Details, On the Map, Distance to the Venue, Weather, Advice for the Couple, quote, footer). The "Our Story" CTA card is gated behind `SHOW_OUR_STORY_CTA` (currently `false`). |
...
- `app/page.tsx` — composes the home page (header → countdown cards → Milestones/By the Numbers → Event Details → On the Map → Distance to the Venue → Weather → Advice → quote → footer). The "Punith & Pallavi" heading is rendered by `<PetalEasterEgg />` (tap it to rain petals). The "Our Story" CTA card (gated by `SHOW_OUR_STORY_CTA`) sits before the quote when enabled.
...
- `app/globals.css` — Tailwind directives + Cormorant Garamond / Inter fonts (Google Fonts `@import` MUST come before the `@tailwind` directives — Turbopack enforces strict CSS ordering). Also holds the `petal-fall` keyframes + `.petal` class for the header easter egg (appended after `@tailwind`, which is safe ordering), with a `prefers-reduced-motion` guard.
- `components/PetalEasterEgg.tsx` — client component rendering the "Punith & Pallavi" heading; tapping it fires a burst of CSS-animated rose petals (26/tap, auto-cleaned after 6s). **Overlay is `createPortal`'d to `document.body`** — required because the heading sits inside `<header class="fade-in">`, whose lingering transform would otherwise clip a `fixed inset-0` overlay to the header (same gotcha as the modals — see "Modal & lightbox layout pattern"). Per-petal `--drift`/`--spin`/size/duration randomized inline; honors `prefers-reduced-motion` (burst no-ops + CSS hides petals).
...
- `components/VenueDistance.tsx` — **Distance to the Venue** section (client component): opt-in geolocation. A "Show my distance" button calls `navigator.geolocation.getCurrentPosition` (no auto-prompt), then computes **straight-line haversine** distance to each venue entirely client-side (coords never stored/sent). Handles `idle / loading / ready / denied / error / unsupported` states. Needs a secure context (HTTPS or localhost — both dev and Vercel qualify). `fmtDistance` switches m/km by magnitude.
...
- Added a **Distance to the Venue** section (`components/VenueDistance.tsx`) after On the Map: opt-in geolocation → client-side haversine distance to each venue (privacy-preserving — coords never leave the browser); graceful denied/error/unsupported states.
- Added a **petal easter egg** (`components/PetalEasterEgg.tsx` + `petal-fall` keyframes in `globals.css`): tapping the "Punith & Pallavi" heading rains rose petals. Client component; overlay portaled to `document.body` to dodge the `.fade-in` transform/containing-block trap; randomized per-petal motion; respects `prefers-reduced-motion`. The home-page heading markup moved from inline `<h1>` into this component.
...
- **Engagement time corrected to 10:00 AM** (was 11:30 AM, briefly 10:30 AM). Updated the `time` field in `components/EventDetails.tsx`, and the engagement `targetISO`/`target`/`ENGAGEMENT` to `2026-06-21T10:00:00` in `app/page.tsx`, `app/wallpaper/page.tsx`, `app/wallpaper-two/page.tsx`, and `components/Journey.tsx` so the countdown and milestone pass-state land on the real start time (matching how the wedding card uses its actual ceremony time). Wedding time unchanged.
- **Wired up the missing `passedVerb` props on the home-page countdown cards.** `app/page.tsx` was rendering `<Countdown>` without `passedVerb`, so once the engagement date passed (now July 2026, engagement was Jun 21) the card fell into a broken half-state: eyebrow showed ✓ but still read "THE PROMISE", digits counted up, and the bottom line showed the stale "The day has arrived." instead of "and counting…". Added `passedVerb="Engaged"` / `passedVerb="Married"` (matching what `/wallpaper` already passed) — the engagement card now reads "✓ ENGAGED / Engaged for / 30 days … / and counting…".
- **Gave the `/wallpaper-two` flip-clock pass-state too.** Its self-contained `CountdownPanel` previously clamped elapsed time to zero (`Math.max(0, …)`), so after a date passed it just showed `000` days / `00:00:00`. Changed `getTime()` to return a signed `passed` flag + absolute elapsed time (count-up), added a `passedVerb` prop to `CountdownPanel` (wired `"Engaged"` / `"Married"` at the two call sites); once passed the eyebrow badge shows `✓ {passedVerb}` (e.g. "✓ ENGAGED") and an "and counting…" caption renders under the H:M:S row. The date heading ("21st June") and the passed panel's count-up now match the classic wallpaper's behavior.
- **Added an optional "X months, Y days left" caption to `Countdown.tsx`** via a new `showMonthsLeft?: boolean` prop (default off, so the home page is unaffected) — enabled only on `/wallpaper` for both cards. Computed with calendar-based `monthsAndDaysBetween()` (calendar Y/M/D components, not a raw `totalMs / 30` approximation, so month length is respected), rendered only while `!isPast` — it silently no-ops once a date passes (e.g. the already-engaged card), since the existing "and counting…" line already covers that case. Renders in the same bottom-caption slot, e.g. "6 months, 21 days left" under the Wedding Day tiles.
