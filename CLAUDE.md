...
- Our Story is sorted **ascending** (Day 1 first, latest at the bottom). Day badge = `index + 1`. The "Day 1 — where it began" anchor at the top derives its date from `moments[0]`. (Earlier we briefly tried newest-first; user explicitly requested Day 1 at top so the chapters read like a book.)
...
- **Digit tick animation:** the countdown digits each have a `key={value}` so React remounts the span on change, triggering the `.animate-tick` keyframe in `globals.css` (320ms slide-up + fade-in + brief blur). The tile container has `overflow-hidden` so the slide doesn't leak. Seconds animate every tick, larger units only when they actually change.
